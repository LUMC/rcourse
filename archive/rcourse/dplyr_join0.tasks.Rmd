...
1. The `name` and `span2` where not good combinations as a key, show the rows with identical 
 `name` and `span2` values.
 
```{r}
dups <- duplicated(survey %>% select(name,span2))
left_join( survey %>% filter(dups) %>% select(name,span2), survey, c("name","span2")) 
```
