...
Course <- R6Class("Course",
    ...
    public = list(
      ...
      initialize = function(path, config="config.yml", site="docs") {
        # Course config file (YAML)
        config <- yaml.load_file(file.path(path,config))
        index_html <- config[["index_file"]] # todo: prefix S01L01l is generated by rendering, but is fixed in config!  
        self$path_ <- path
      ...
      render = function(clean=FALSE,...){
        renderer <- Renderer$new( outDir = "docs" )  # paste0( self$path_, ".site" )
