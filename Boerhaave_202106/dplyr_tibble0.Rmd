...
I'll use the terms *observations* and *rows* interchangeably depending on the context. The same goes for the terms *variables* and *columns*. Datasets `pulse` and `survey` are of type tibble. Each variable in a tibble has a fixed [type](S02L01l_basic_vectors0.html) such as `character`, `double` etc. Let's start by creating a tibble manually.
...
- **bind_rows(...)**  function creates a new tibble from a set of tibbles with common variables. 
...
`r info_block("Only tibbles with identical set of variables or observations may be bound.")`
