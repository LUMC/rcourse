...
1. *(ADV) Simulate grades. (Calling user function for each element.)*  
Consider the `scores` list from the previous exercises.  
...
Hints:
    ...
    - Put above to a function `genGrades` that generates grades for a single student.
    - Use `lapply` to apply the function to each element of `nms`.
...
2. *(ADV) Split a table into list of tables by a column factor; merge back*.  
...
3. *(ADV) Split a table by a column and write each part to a different file*.  
