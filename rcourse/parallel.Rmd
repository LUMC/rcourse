...
myCalculation <- function( pars ) {
    ...
    res$waitTime <- sample(1:5, 1)
    res$x <- sample(1:10, 1)
    res$y <- sample(LETTERS, 1)
    prefix <- rep( "   ", res$waitTime )
    .MESSAGE( prefix, "Starting calculation with parameters: ", paste0(pars, collapse=", "), " ", res$waitTime, "\n" )
    Sys.sleep( res$waitTime )
    .MESSAGE( prefix, "Calculation finished with result: ", res$waitTime, "\n" )
    res
}
```

## Forking (Linux/Mac only)

```{r}
parallel::mclapply( 1:5, myCalculation, mc.cores = 10 )
```
...
parsList <- lapply( 1:20, function(i) list( a=i, b=i+1, c=i+2 ) )
res <- parLapply( cl, parsList, myCalculation )
stopCluster( cl )

res |> map( as_tibble ) |> bind_rows()
```

## Explanation

The `makeCluster` function creates a cluster of workers.
The `parLapply` function applies a function to each element of a list in parallel.
The `stopCluster` function stops the cluster of workers.
