...
4. Recall `Data manipulation 1/3, Tibble exercise 4.2`, where you were asked to add a new variable `height`. Repeat the exercise by using a `join` functions.
...
left_join(favourite_colour1, heights,"name")
```
...
3.1 Add `Isaac` with age `23`. 
```{r}
bind_rows(favourite_colour, tibble(name="Isaac",age=23 ))
```

3.2 Include heights `175cm` and `183cm` for `Lotte` and `Lucas` respectively.
...
bind_cols(favourite_colour, tibble(height=c(183,175, NA, NA)))
```
