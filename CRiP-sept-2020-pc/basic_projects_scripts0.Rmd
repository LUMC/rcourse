...
From the RStudio menu choose: `File`/`New project...`/`New directory...`/`New project...`  
...
Look at the top right corner of the RStudio window to check which project you are working in.  
Refer to the following Figure: it shows a state when a project called `Learning_R` is open.

Moreover, the title line shows **the project diectory** in which the project files are located.
...
In this course R scripts are not used.

## R Markdown documents

[**R Markdown** document](https://rmarkdown.rstudio.com/) (e.g. `filename.Rmd`) is an extension of R scripts.  
R Markdown is a simple text file, which in addition to R language commands might contain any free text (prose; or like the sentence which you are reading right now).  
The free text might be decorated (contain: sections, lists, tables, [hyperlinks](https://rmarkdown.rstudio.com/docs/), formulae: $r=\sqrt{x^2+y^2}$, etc.).

R Markdown documents are **knitted** to produce the report file.  
Many report formats are available: HTML, PDF (printable pages or presentations), Microsoft Word documents.

Watch the [R Markdown introduction video](https://rmarkdown.rstudio.com/lesson-1.html) from RStudio.

## R Markdown text decorations

To learn how to decorate text using R Markdown follow the lessons available at [https://www.markdowntutorial.com/](https://www.markdowntutorial.com/).
