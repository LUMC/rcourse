...
- RStudio highlights the chunk lines gray.

`r direct_exercise( "Locate two chunks in the Figure below. Locate a chunk (with a more complex R code) in the Figure above." )`

`r direct_exercise( "Type a new chunk in your R Markdown document, based on the Figure below. Put a simple R expression into the chunk. Knit to see the effect." )`

`r direct_exercise( "Add a new chunk with another R expression. Press the green play button in the upper right corner of the chunk area. Change the expression. Press play again." )`
...
To run a chunk you may use `Ctrl-Enter` key combination.

Note the difference:

- When you run a chunk it may use the variables currently present in memory (see `Environment`); this is good when you _develop_ your report.
- `Knit` button processes the whole R Markdown with no variables in memory (empty `Environment`); this is _necessary for the final version_ of your report.

## Editing  `r todo( "Write own chunks" )`
