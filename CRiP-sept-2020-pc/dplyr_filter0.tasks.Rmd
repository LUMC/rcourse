...
```{r}
filter(survey, 9<age & age<20) # <=> filter(survey, 10<=age & age<=19)
```
...
```{r}
# men
none_smoker_sportsmen <- nrow( filter(survey, smokes=="never" & exercise=="freq" & gender=="male") )
total_men <- nrow( filter(survey, gender=="male") ) 
none_smoker_sportsmen/total_men
# women
none_smoker_sportswomen <- nrow( filter(survey, smokes=="never" & exercise=="freq" & gender=="female") )
