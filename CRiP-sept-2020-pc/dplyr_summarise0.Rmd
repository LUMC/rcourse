...
Multiple variables are allowed, it is the count of each possible combination of values, also known as `contigency table` or `cross table`: 
...
to identify distinct combinations of `gender` and `exercise`:
...
You may use distinct also to check whether certain variables have unique values for each observation. Let's for example check whether all individuals in the `pulse` dataset have different names:
...
There are `r pulse %>% distinct(name) %>% nrow()` distinct names and there are `r nrow(pulse)` observations in the `pulse` dataset. This could only mean that there are multiple individuals in the pulse dataset with shared names: 
...
With `glimpse` function you can quickly inspect the top part of all variables in one page with some additional met information, i.e. number of rows and columns and variable types: 
...
You may reorder rows by one or more variables with `arrange`.  
