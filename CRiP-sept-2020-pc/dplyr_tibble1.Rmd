...
A tibble is the upgraded version of [data.frame]() data type with some convenient additional features, moreover, it is the standard data structure in [tidyverse](https://www.tidyverse.org/). In the following I'll layout the main features and differences in comparison with data.frame. Let's create a data.frame and a tibble manually with the same content.
...
You can see that the output layout has changed, it does not report the dimensions and variable types, and you can check the type which pure `data.frame` without `tbl_df` and `tbl`:    
