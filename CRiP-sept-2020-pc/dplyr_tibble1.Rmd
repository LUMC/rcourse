...
A tibble is the upgraded version of [data.frame]() data type with some convenient additional features, moreover, it is the standard data structure in tidyverse. In the following I'll layout the main features and differences in comparison with data.frame. Let's first create a data frame manually.

## Create data frame

Creating a __tibble__ is identical to creating a __data.frame__. They both take a list of `key=value` pairs separated by comma with `key` being the name and `value` the values of the column. Take for example the variables `name`, `year` and `favourite` to represent a person's name, birth year and favourite colour:   

__data.frame__
```{r, echo=TRUE }
t1 <- data.frame(name=c("Lucas","Lotte","Noa"), 
                 year=1995, 
                 favorite=c("Blue","Green","Yellow"))
t1
```
...
Note that as_tibble maintains the original column types, so name and favourite are still factors even after conversion to tibble. The reverse is also possible, converting tibble to data.frame:
