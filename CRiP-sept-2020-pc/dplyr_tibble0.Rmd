...
any [atomic type]() such as character,integer etc. Let's start by creating one manually.
...
The object `favourite_color` now holds the data, type it's name in the R console for inspection:
...
msg <- "1) '# A tibble: 4 x 3', which says that this is a tibble with 4x3 dimensions (4 observations 
...
enforced by {`tbl_df`,`tbl`} type. For more details on the differences between tibble and data.frame 
...
For more details on reading/writing format see [readr](readr_io0.html).
...
`favourite_color` tibble is written to `favourite_color.csv` text file. You may inspect the file with any editor and it should look something like: 
