...
This is a wrong way to "increase _x_ by one", because the result is calculated but not stored:
...
Naturally, there are some limitations. Do not use:

- names containing a space
- names containing any of `@#!%$^()-+=!~?,<>}{][`
- names which are R reserved names: `for`, `while`, `repeat`, `if`, `else`, `TRUE`, `FALSE`, `Inf`, `NA`, `NaN`
