...
In a variable `scores` create a `list` (the names of the list elements should be names of the students and the values should be the corresponding grades).  
...
Then, for Dan new grades (8,8,6,7) need to be added (hint: use the combine function `c` to combine existing Dan's grades with the new grades; then put the result back to `scores`).
```{r}
...
3. *A list returned from a function*.  
