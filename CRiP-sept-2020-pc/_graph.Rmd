...
related_slots <- lapply(names(slots), function(slot) {
  related_slots <- slots[[slot]][["related"]] 
  if (!is.null(related_slots)) {
    related_slots <- strsplit(related_slots," ")[[1]] 
  } else {
    related_slots <- NULL
  }
  lapply(related_slots, function(x) {
...
edges <- bind_rows(related_slots, next_slots) %>% rename(from=source,to=target)
...
visNetwork(nodes, edges,width = "100%" ) %>% 
  ...
  visEdges(arrows = list(to=list( scaleFactor=.5)),
           ...
           visEvents(doubleClick = "function(e) {
