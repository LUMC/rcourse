...
Here $f$ and $g$ are functions and $x$ is a data object, possibly a vector but in majority of cases a tibble. The left hand side of the $f(g(x))$ is the standard in R, it calls first the $g(x)$ and then the result of $g(x)$ is taken as input to the function $f$. The right hand side, $x \pipe g \pipe f$,  will lead to the same result as $f(g(x))$.  
