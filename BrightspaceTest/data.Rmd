...
The pulse data set is available in the data folder as comma-delimited text (extension `.csv`) file `pulse.csv`. It is also available in SPSS-format as `pulse.sav`.
...
For SPSS files (extension `.sav`) we need the function `read_spss` from the R package `haven`. Try `library(haven)`, if the package is not installed then you'll get an error `threre is no package called 'haven'`, in that case you'll need to install it first using:
